...
function NuevaEmpresaModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><div className="w-full max-w-2xl rounded-xl bg-white p-6 shadow-2xl"><h3 className="text-xl font-bold text-[#052A79]">Nueva Empresa</h3><p className="mb-5 text-sm text-slate-500">La empresa será propia de FAREGAS y se creará activa.</p><form onSubmit={guardar} className="space-y-4"><div className="grid gap-4 md:grid-cols-2"><div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Código</label><input required maxLength={30} value={form.key} onChange={(event) => setForm({ ...form, key: event.target.value.toUpperCase().replace(/[^A-Z0-9_]/g, '') })} className="w-full rounded-lg border border-slate-300 p-2.5 font-mono font-bold" placeholder="EMPRESA_NUEVA" /></div><div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">RUC</label><input inputMode="numeric" maxLength={11} value={form.ruc} onChange={(event) => setForm({ ...form, ruc: event.target.value.replace(/\D/g, '') })} className="w-full rounded-lg border border-slate-300 p-2.5 font-mono" /></div></div><div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Razón social</label><input required value={form.nombre} onChange={(event) => setForm({ ...form, nombre: event.target.value })} className="w-full rounded-lg border border-slate-300 p-2.5" /></div><div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Dirección</label><input value={form.direccion} onChange={(event) => setForm({ ...form, direccion: event.target.value })} className="w-full rounded-lg border border-slate-300 p-2.5" /></div><div className="grid gap-4 md:grid-cols-2"><div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Teléfono</label><input value={form.telefono} onChange={(event) => setForm({ ...form, telefono: event.target.value })} className="w-full rounded-lg border border-slate-300 p-2.5" /></div><div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Cuenta Banco de la Nación</label><input value={form.cuenta_banco_nacion} onChange={(event) => setForm({ ...form, cuenta_banco_nacion: event.target.value })} className="w-full rounded-lg border border-slate-300 p-2.5 font-mono" /></div></div>{error && <div className="rounded-lg bg-red-50 p-3 text-sm font-semibold text-red-700">{error}</div>}<div className="flex justify-end gap-3 border-t pt-4"><button type="button" disabled={saving} onClick={onClose} className="rounded-lg px-5 py-2 font-semibold text-slate-600 hover:bg-slate-100">Cancelar</button><button disabled={saving} className="rounded-lg bg-[#052A79] px-5 py-2 font-bold text-white disabled:opacity-50">{saving ? 'Creando...' : 'Crear Empresa'}</button></div></form></div></div>;
}
...
function EmpresaModal({ empresa, onClose, onSaved }: { empresa: EmpresaFaregas; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><div className="w-full max-w-2xl rounded-xl bg-white p-6 shadow-2xl"><h3 className="text-xl font-bold text-[#052A79]">Editar Empresa</h3><p className="mb-5 text-sm text-slate-500">Código inmutable: <span className="font-mono font-bold">{empresa.key}</span></p><form onSubmit={guardar} className="space-y-4"><div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Razón social</label><input required value={form.nombre} onChange={(event) => setForm({ ...form, nombre: event.target.value })} className="w-full rounded-lg border border-slate-300 p-2.5" /></div><div className="grid gap-4 md:grid-cols-2"><div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">RUC</label><input inputMode="numeric" maxLength={11} value={form.ruc} onChange={(event) => setForm({ ...form, ruc: event.target.value.replace(/\D/g, '') })} className="w-full rounded-lg border border-slate-300 p-2.5 font-mono" /></div><div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Teléfono</label><input value={form.telefono} onChange={(event) => setForm({ ...form, telefono: event.target.value })} className="w-full rounded-lg border border-slate-300 p-2.5" /></div></div><div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Dirección</label><input value={form.direccion} onChange={(event) => setForm({ ...form, direccion: event.target.value })} className="w-full rounded-lg border border-slate-300 p-2.5" /></div><div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Cuenta Banco de la Nación</label><input value={form.cuenta_banco_nacion} onChange={(event) => setForm({ ...form, cuenta_banco_nacion: event.target.value })} className="w-full rounded-lg border border-slate-300 p-2.5 font-mono" /></div>{error && <div className="rounded-lg bg-red-50 p-3 text-sm font-semibold text-red-700">{error}</div>}<div className="flex justify-end gap-3 border-t pt-4"><button type="button" disabled={saving} onClick={onClose} className="rounded-lg px-5 py-2 font-semibold text-slate-600 hover:bg-slate-100">Cancelar</button><button disabled={saving} className="rounded-lg bg-[#052A79] px-5 py-2 font-bold text-white disabled:opacity-50">{saving ? 'Guardando...' : 'Guardar Empresa'}</button></div></form></div></div>;
}
