...
export default function NubefactReadinessPanel() {
  ...
  const cargar = useCallback(async () => {
    try {
      ...
      setData(await faregasNubefactReadinessApi.obtenerPanel());
    } catch (err) {
  ...
  return <div className="space-y-5">
    <div className={`rounded-xl border-2 p-5 ${data.estado === 'LISTO' ? 'border-green-300 bg-green-50' : 'border-amber-300 bg-amber-50'}`}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div><h3 className="font-black text-slate-800">Preparación técnica de Nubefact</h3><p className="text-sm text-slate-600">Diagnóstico de solo lectura; no emite ni reserva correlativos.</p></div>
        </div>
    ...
    <section className="rounded-xl border bg-white p-5">
      ...
      <div className="grid gap-3 md:grid-cols-4"><Card label="Aceptados" value={data.monitoreo.aceptados} tone="green" /><Card label="Pendientes" value={data.monitoreo.pendientes} tone="amber" /><Card label="Errores" value={data.monitoreo.errores} tone="red" /><Card label="Rechazados" value={data.monitoreo.rechazados} tone="red" /></div>
    </section>
  </div>;
}
