...
const parseCsv = (content: string): CatalogoFiscalImportRow[] => {
  ...
  const plantaIndex = headers.indexOf('planta_key');
  const servicioIndex = headers.indexOf('servicio_codigo');
  const skuIndex = headers.indexOf('codigo_sku');
  const tarifaIndex = headers.indexOf('tarifa_id');
  if (plantaIndex < 0 || servicioIndex < 0 || skuIndex < 0) {
    throw new Error('Se requieren las columnas planta_key, servicio_codigo y codigo_sku.');
  }
  return lines.slice(1).map(line => {
    const columns = line.split(delimiter).map(value => value.trim().replace(/^"|"$/g, ''));
    return {
      planta_key: columns[plantaIndex] || '',
      servicio_codigo: columns[servicioIndex] || '',
      codigo_sku: columns[skuIndex] || '',
      ...(tarifaIndex >= 0 && columns[tarifaIndex] ? { tarifa_id: Number(columns[tarifaIndex]) } : {})
    };
  });
};

const descargarPlantilla = (tarifas: TarifaPlantilla[]) => {
  const content = [
    'planta_key;servicio_codigo;codigo_sku;tarifa_id',
    ...tarifas.map(item => `${item.plantaKey};${item.servicioCodigo};${item.sku};${item.id}`)
  ].join('\r\n');
...
export default function CatalogoFiscalImportModal({ tarifas, onClose, onApplied }: { tarifas: TarifaPlantilla[]; onClose: () => void; onApplied: () => Promise<void> }) {
  const [rows, setRows] = useState<CatalogoFiscalImportRow[]>([]);
  ...
  const aplicar = async () => {
    ...
    if (!window.confirm(`Se actualizarán ${preview.cambios} tarifas; ${preview.sinCambios} ya estaban vinculadas. ¿Confirmar?`)) return;
    try {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><div className="max-h-[92vh] w-full max-w-4xl overflow-auto rounded-xl bg-white p-6 shadow-2xl">
    ...
    <div className="rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900"><strong>Formato compatible con Excel:</strong> complete <code>planta_key</code>, <code>servicio_codigo</code> y <code>codigo_sku</code>. <code>tarifa_id</code> es un control adicional. La previsualización no modifica datos.</div>
    ...
    {preview && <><div className="mb-3 flex flex-wrap gap-3 text-sm"><span className="rounded bg-slate-100 px-3 py-1 font-bold">Total {preview.total}</span><span className="rounded bg-green-100 px-3 py-1 font-bold text-green-700">Cambios {preview.cambios}</span><span className="rounded bg-blue-100 px-3 py-1 font-bold text-blue-700">Sin cambios {preview.sinCambios}</span><span className="rounded bg-red-100 px-3 py-1 font-bold text-red-700">Inválidas {preview.invalidas}</span></div><div className="max-h-80 overflow-auto rounded-lg border"><table className="w-full text-left text-xs"><thead className="sticky top-0 bg-slate-100"><tr><th className="p-3">Fila</th><th className="p-3">Sede</th><th className="p-3">Servicio</th><th className="p-3">SKU</th><th className="p-3">Estado</th><th className="p-3">Detalle</th></tr></thead><tbody>{preview.filas.map(row => <tr key={`${row.fila}-${row.plantaKey}-${row.servicioCodigo}`} className="border-t"><td className="p-3">{row.fila}</td><td className="p-3">{row.tarifa?.plantaNombre || row.plantaKey}</td><td className="p-3">{row.tarifa?.servicioNombre || row.servicioCodigo}</td><td className="p-3 font-mono">{row.producto?.sku || row.productoSku}</td><td className={`p-3 font-bold ${row.estado === 'INVALIDA' ? 'text-red-700' : row.estado === 'SIN_CAMBIOS' ? 'text-blue-700' : 'text-green-700'}`}>{row.estado}</td><td className="p-3">{row.errores.join(' · ') || row.producto?.descripcion}</td></tr>)}</tbody></table></div></>}
    <div className="mt-6 flex justify-end gap-3 border-t pt-4"><button type="button" onClick={onClose} disabled={loading} className="rounded-lg px-5 py-2 font-bold text-slate-600">Cancelar</button><button type="button" onClick={() => void aplicar()} disabled={loading || !preview || preview.invalidas > 0 || preview.cambios === 0} className="rounded-lg bg-[#052a79] px-5 py-2 font-bold text-white disabled:opacity-40">APLICAR VINCULACIONES</button></div>
  </div></div>;
}
