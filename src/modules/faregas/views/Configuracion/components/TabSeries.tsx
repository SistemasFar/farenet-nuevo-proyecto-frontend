...
function SerieModal({ estado, sede, onClose, onSaved }: { estado: NonNullable<ModalState>; sede: SerieSede; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const [error, setError] = useState('');
    ...
    return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 overflow-y-auto"><div className="w-full max-w-xl rounded-xl bg-white p-6 shadow-2xl my-8"><h3 className="text-xl font-bold text-[#052A79]">{estado.modo === 'CREAR' ? 'Nueva Serie Nubefact' : 'Editar Serie'}</h3><p className="mb-5 text-sm text-slate-500">Serie tributaria exclusiva; nunca se comparte con DMS Fact ni FARENET.</p><form onSubmit={guardar} className="space-y-4"><div><label className="mb-1 block text-xs font-bold uppercase text-slate-500">Sede</label><input readOnly value={sede.nombre} className="w-full rounded-lg border bg-slate-100 p-2.5" /></div>{estado.modo === 'CREAR' && <div><label className="mb-1 block text-xs font-bold uppercase text-slate-500">Ambiente Nubefact</label><select value={entornoEmision} onChange={(e) => setEntornoEmision(e.target.value as 'DEMO' | 'PRODUCCION')} className="w-full rounded-lg border bg-white p-2.5 font-bold"><option value="DEMO">DEMO — pruebas sin valor tributario</option><option value="PRODUCCION">PRODUCCIÓN — requiere validación y corte formal</option></select></div>}<div className="grid gap-4 md:grid-cols-2"><div><label className="mb-1 block text-xs font-bold uppercase text-slate-500">Tipo comprobante</label><select disabled={estado.modo === 'EDITAR'} value={tipo} onChange={(e) => setTipo(e.target.value as TipoComprobanteFaregas)} className="w-full rounded-lg border bg-white p-2.5 disabled:bg-slate-100">{TIPOS_COMPROBANTE.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></div><div><label className="mb-1 block text-xs font-bold uppercase text-slate-500">Serie</label><input required maxLength={4} minLength={4} readOnly={estado.modo === 'EDITAR'} value={serie} onChange={(e) => setSerie(e.target.value.toUpperCase())} className="w-full rounded-lg border p-2.5 font-mono font-bold read-only:bg-slate-100" /></div></div>
