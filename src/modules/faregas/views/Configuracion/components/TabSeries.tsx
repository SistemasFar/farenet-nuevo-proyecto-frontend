...
export default function TabSeries() {
  ...
  const [entorno, setEntorno] = useState<'DEMO' | 'PRODUCCION'>('DEMO');
  ...
  const cargarSeries = async (key = plantaKey) => {
    if (!key) return;
    try { setLoading(true); setError(''); const data = await faregasSeriesApi.listar(key); setSeries(data.series); setMigracionAplicada(data.migracionNubefactAplicada); }
  ...
  useEffect(() => {
    ...
    void faregasSeriesApi.listar(plantaKey)
      .then((data) => { if (!cancelado) { setSeries(data.series); setMigracionAplicada(data.migracionNubefactAplicada); setError(''); } })
  ...
  const filtradas = useMemo(() => series.filter((item) => {
    ...
    return (!texto || item.serie.toLowerCase().includes(texto))
      ...
      && (item.proveedor_emision !== 'NUBEFACT' || item.entorno_emision === entorno);
  }), [series, buscar, tipo, estado, entorno]);
  const sinPredeterminada = TIPOS_COMPROBANTE.map((item) => item.value).filter((item) =>
    !series.some((serie) => serie.tipo_comprobante === item && serie.activo && serie.es_predeterminada
      && serie.proveedor_emision === 'NUBEFACT' && serie.entorno_emision === entorno)
  );
  ...
  return <div>
    <div className={`mb-4 rounded-xl border p-4 text-sm ${migracionAplicada ? 'border-blue-300 bg-blue-50 text-blue-900' : 'border-amber-300 bg-amber-50 text-amber-900'}`}><strong>{migracionAplicada ? 'Series Nubefact aisladas por ambiente.' : 'Migración tributaria pendiente.'}</strong> {migracionAplicada ? 'El motor V2 solo utilizará filas marcadas como NUBEFACT del ambiente seleccionado; las series LEGACY no son elegibles.' : 'Las filas actuales siguen siendo LEGACY y no serán utilizadas por el motor V2. No se pueden crear series Nubefact hasta aplicar la migración autorizada.'}</div>
    <div className="mb-5 flex flex-col gap-3 xl:flex-row xl:items-end"><div className="min-w-64"><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Sede</label><select value={plantaKey} onChange={(e) => { setLoading(true); setPlantaKey(e.target.value); }} className="w-full rounded-lg border border-slate-300 bg-white p-2.5 text-sm font-semibold focus:border-[#052A79] focus:outline-none">{sedes.map((item) => <option key={item.key} value={item.key}>{item.nombre}{item.activo ? '' : ' (INACTIVA)'}</option>)}</select></div><select value={entorno} onChange={(e) => setEntorno(e.target.value as 'DEMO' | 'PRODUCCION')} className="rounded-lg border border-slate-300 bg-white p-2.5 text-sm font-bold"><option value="DEMO">Ambiente: DEMO</option><option value="PRODUCCION">Ambiente: PRODUCCIÓN</option></select><select value={tipo} onChange={(e) => setTipo(e.target.value)} className="rounded-lg border border-slate-300 bg-white p-2.5 text-sm focus:border-[#052A79] focus:outline-none"><option value="">Tipo: Todos</option>{TIPOS_COMPROBANTE.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><select value={estado} onChange={(e) => setEstado(e.target.value)} className="rounded-lg border border-slate-300 bg-white p-2.5 text-sm focus:border-[#052A79] focus:outline-none"><option value="">Estado: Todos</option><option value="1">Activas</option><option value="0">Inactivas</option></select><div className="flex-1"><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Buscar</label><input value={buscar} onChange={(e) => setBuscar(e.target.value)} placeholder="Buscar serie" className="w-full rounded-lg border border-slate-300 p-2.5 text-sm focus:border-[#052A79] focus:outline-none" /></div><button disabled={!plantaKey || !migracionAplicada} title={migracionAplicada ? 'Crear serie exclusiva Nubefact' : 'Migración Nubefact pendiente'} onClick={() => setModal({ modo: 'CREAR' })} className="rounded-lg bg-[#052A79] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50">+ NUEVA SERIE NUBEFACT</button></div>
    {migracionAplicada && sinPredeterminada.length > 0 && <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm font-semibold text-amber-800">Sin serie Nubefact predeterminada en {entorno}: {sinPredeterminada.map(etiquetaTipo).join(', ')}. El motor V2 bloqueará estos documentos.</div>}
        ...
        <div className="max-h-[58vh] overflow-auto">
          <table className="min-w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">{filtradas.map((serie) => {
              ...
              return <tr key={serie.id} className="hover:bg-slate-50">
                <td className="px-4 py-3 font-mono text-base font-bold text-[#052A79]">{serie.serie}{serie.contingencia && <span className="ml-2 rounded bg-amber-100 px-2 py-0.5 text-xs text-amber-700">CONTINGENCIA</span>}</td>
                <td className="px-4 py-3 font-semibold">{detalle.documento}<div className="text-xs font-normal text-slate-400">{detalle.referencia !== '—' ? `Referencia: ${detalle.referencia}` : ''}</div></td>
                <td className="px-4 py-3"><span className={`rounded px-2 py-1 text-xs font-bold ${esNubefact ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>{esNubefact ? 'NUBEFACT EXCLUSIVA' : 'LEGACY / FARENET'}</span>{esNubefact && <div className="mt-1 text-xs font-bold text-slate-500">{serie.entorno_emision}</div>}</td>
                <td className="px-4 py-3 text-right font-mono font-bold">{serie.ultimo_numero.toLocaleString('es-PE')}</td>
                <td className="px-4 py-3 text-center">{!puedeConfirmar ? <span className="text-xs text-slate-400">NO APLICA</span> : serie.confirmada_produccion ? <span className="rounded bg-green-100 px-2 py-1 text-xs font-bold text-green-700">CONFIRMADA</span> : <span className="rounded bg-amber-100 px-2 py-1 text-xs font-bold text-amber-700">PENDIENTE</span>}</td>
                <td className="px-4 py-3 text-center">{serie.serie_pos ? <span className="rounded bg-teal-100 px-2 py-1 text-xs font-bold text-teal-700">SÍ</span> : 'No'}</td>
                <td className="px-4 py-3 text-center"><span className={`rounded-full px-2 py-1 text-xs font-bold ${serie.activo ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{serie.activo ? 'ACTIVA' : 'INACTIVA'}</span></td>
                <td className="px-4 py-3 text-center"><div className="flex justify-center gap-2"><button onClick={() => setModal({ modo: 'EDITAR', serie })} title="Editar" className="rounded-md border border-blue-200 bg-blue-50 px-2.5 py-1.5 text-[#052A79] hover:bg-blue-100"><Edit size={18} /></button>{puedeConfirmar && <button onClick={() => setConfirmarSerie(serie)} className={serie.confirmada_produccion ? 'text-amber-700' : 'text-green-700'}>{serie.confirmada_produccion ? 'Revocar producción' : 'Confirmar producción'}</button>}<button onClick={() => void cambiarEstado(serie)} title={serie.activo ? 'Desactivar' : 'Activar'} className={serie.activo ? 'rounded-md border border-red-200 bg-red-50 px-2.5 py-1.5' : 'rounded-md border border-green-200 bg-green-50 px-2.5 py-1.5'}>{serie.activo ? <PowerOff size={18} /> : <Power size={18} />}</button></div></td>
              </tr>;
            })}</tbody>
          </table>
        </div>}
...
function SerieModal({ estado, sede, onClose, onSaved }: { estado: NonNullable<ModalState>; sede: SerieSede; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const [entornoEmision, setEntornoEmision] = useState<'DEMO' | 'PRODUCCION'>(actual?.entorno_emision || 'DEMO');
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
    ...
    const guardar = async (event: React.FormEvent) => {
      ...
      try {
        setSaving(true);
        if (estado.modo === 'CREAR') await faregasSeriesApi.crear({ planta_key: sede.key, tipo_comprobante: tipo, serie: serie.trim().toUpperCase(), ultimo_numero: Number(ultimoNumero), es_predeterminada: predeterminada, autogenerada, contingencia, activo, proveedor_emision: 'NUBEFACT', entorno_emision: entornoEmision, tipo_documento_referencia: tipoRef, serie_pos: seriePos });
    ...
    return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 overflow-y-auto"><div className="w-full max-w-xl rounded-xl bg-white p-6 shadow-2xl my-8"><h3 className="text-xl font-bold text-[#052A79]">{estado.modo === 'CREAR' ? 'Nueva Serie Nubefact' : 'Editar Serie'}</h3><p className="mb-5 text-sm text-slate-500">Serie tributaria exclusiva; nunca se comparte con DMS Fact ni FARENET.</p><form onSubmit={guardar} className="space-y-4"><div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Sede</label><input readOnly value={sede.nombre} className="w-full rounded-lg border bg-slate-100 p-2.5" /></div>{estado.modo === 'CREAR' && <div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Ambiente Nubefact</label><select value={entornoEmision} onChange={(e) => setEntornoEmision(e.target.value as 'DEMO' | 'PRODUCCION')} className="w-full rounded-lg border bg-white p-2.5 font-bold"><option value="DEMO">DEMO — pruebas sin valor tributario</option><option value="PRODUCCION">PRODUCCIÓN — requiere validación y corte formal</option></select></div>}<div className="grid gap-4 md:grid-cols-2"><div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Tipo comprobante</label><select disabled={estado.modo === 'EDITAR'} value={tipo} onChange={(e) => setTipo(e.target.value as TipoComprobanteFaregas)} className="w-full rounded-lg border bg-white p-2.5 disabled:bg-slate-100">{TIPOS_COMPROBANTE.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></div><div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Serie</label><input required maxLength={4} minLength={4} readOnly={estado.modo === 'EDITAR'} value={serie} onChange={(e) => setSerie(e.target.value.toUpperCase())} className="w-full rounded-lg border p-2.5 font-mono font-bold read-only:bg-slate-100" /></div></div>
    
    {esNota && (
      <div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Documento de Referencia (Para Notas)</label><select value={tipoRef} onChange={(e) => setTipoRef(e.target.value)} className="w-full rounded-lg border bg-white p-2.5"><option value="">-- Seleccionar (Opcional) --</option><option value="01">Factura (01)</option><option value="03">Boleta (03)</option></select></div>
    )}
    
    <div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Último número {estado.modo === 'EDITAR' && '(solo lectura)'}</label><input required type="number" min="0" step="1" readOnly={estado.modo === 'EDITAR'} value={ultimoNumero} onChange={(e) => setUltimoNumero(e.target.value)} className="w-full rounded-lg border p-2.5 font-mono read-only:bg-slate-100" /><p className="mt-1 text-xs text-slate-500">Debe ser el último correlativo realmente utilizado. El siguiente reservado será {Number(ultimoNumero || 0) + 1}.</p></div><div className="grid gap-3 rounded-lg border p-4 md:grid-cols-2"><label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={predeterminada} onChange={(e) => setPredeterminada(e.target.checked)} />Predeterminada</label><label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={autogenerada} onChange={(e) => setAutogenerada(e.target.checked)} />Autogenerada</label><label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={contingencia} onChange={(e) => setContingencia(e.target.checked)} />Contingencia</label><label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={seriePos} onChange={(e) => setSeriePos(e.target.checked)} />Serie para POS</label>{estado.modo === 'CREAR' && <label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={activo} onChange={(e) => setActivo(e.target.checked)} />Activa</label>}</div>{error && <div className="rounded-lg bg-red-50 p-3 text-sm font-semibold text-red-700">{error}</div>}<div className="flex justify-end gap-3 border-t pt-4"><button type="button" disabled={saving} onClick={onClose} className="rounded-lg px-5 py-2 font-semibold text-slate-600 hover:bg-slate-100">Cancelar</button><button disabled={saving} className="rounded-lg bg-[#052A79] px-5 py-2 font-bold text-white disabled:opacity-50">{saving ? 'Guardando...' : 'Guardar Serie Nubefact'}</button></div></form></div></div>;
}
