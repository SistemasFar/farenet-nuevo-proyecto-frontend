...
const getTarifaStatus = (tarifa: TarifaAdmin) => {
  ...
  if (
    ...
    (Boolean(tarifa.producto_categoria_id) && tarifa.producto_categoria_id !== tarifa.categoria_id) ||
  ...
  return <div>
    <div className="mb-4 grid grid-cols-2 gap-4 md:grid-cols-4">
      <div className="rounded-xl border bg-white p-4 shadow-sm">
        <div className="text-xs font-bold text-slate-500 capitalize">Total de Tarifas</div>
        <div className="text-2xl font-bold text-slate-800">{stats.total}</div>
      </div>
      <div className="rounded-xl border border-green-200 bg-green-50 p-4 shadow-sm">
        <div className="text-xs font-bold text-green-700 capitalize">Listas para Nubefact</div>
        <div className="text-2xl font-bold text-green-800">{stats.listas}</div>
      </div>
      <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 shadow-sm">
        <div className="text-xs font-bold text-amber-700 capitalize">Incompletas</div>
        <div className="text-2xl font-bold text-amber-800">{stats.incompletas}</div>
      </div>
      <div className="rounded-xl border border-red-200 bg-red-50 p-4 shadow-sm">
        <div className="text-xs font-bold text-red-700 capitalize">Prod. Fiscales Inválidos</div>
    ...
    <div className="mb-5 flex flex-col gap-3 xl:flex-row xl:items-end">
      <div className="min-w-64"><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Sede</label><select value={plantaKey} onChange={(e) => { setLoading(true); setPlantaKey(e.target.value); setCategoria(''); }} className="w-full rounded-lg border border-slate-300 bg-white p-2.5 text-sm font-semibold focus:border-[#052A79] focus:outline-none">{sedes.map((item) => <option key={item.key} value={item.key}>{item.nombre}{item.activo ? '' : ' (INACTIVA)'}</option>)}</select></div>
      <div className="flex-1"><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Buscar</label><input value={buscar} onChange={(e) => setBuscar(e.target.value)} placeholder="Código o nombre de servicio" className="w-full rounded-lg border border-slate-300 p-2.5 text-sm focus:border-[#052A79] focus:outline-none" /></div>
...
const getProductoError = (p: ProductoTarifa, exigeDatosTributarios: boolean, categoriaId?: number, permitirSinCategoria = false) => {
  if (!p.activo) return 'Producto inactivo';
  if (!p.es_para_venta) return 'No habilitado para venta';
  if (!p.categoria_id && !permitirSinCategoria) return 'Producto sin categoría';
  if (categoriaId && p.categoria_id !== categoriaId) return 'Categoría incompatible';
...
function TarifaModal({ estado, sede, onClose, onSaved }: { estado: NonNullable<ModalState>; sede: TarifaSede; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const [producto, setProducto] = useState<ProductoTarifa | null>(tarifa?.producto_facturacion_id ? { 
    ...
    categoria_id: tarifa.producto_categoria_id,
    categoria_codigo: tarifa.producto_categoria_codigo,
    categoria_nombre: tarifa.producto_categoria_nombre
  } : null);
  ...
  const categoriaOperacionId = estado.modo === 'EDITAR' ? tarifa?.categoria_id : servicio?.categoria_id;
  const productoLegacySinCategoria = estado.modo === 'EDITAR'
    && producto?.id === tarifa?.producto_facturacion_id
  ...
  const guardar = async (event: React.FormEvent) => {
    ...
    if (producto) {
      const pErr = getProductoError(producto, exigeDatosTributarios, categoriaOperacionId, productoLegacySinCategoria);
      if (pErr) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"><div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-xl bg-white p-6 shadow-2xl"><h3 className="text-xl font-bold text-[#052A79]">{estado.modo === 'CREAR' ? 'Asignar Servicio' : 'Editar Tarifa'}</h3><p className="mb-5 text-sm text-slate-500">Tarifa operativa oficial de Faregas</p><form onSubmit={guardar} className="space-y-4"><div className="grid gap-4 md:grid-cols-2"><div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Sede</label><input readOnly value={sede.nombre} className="w-full rounded-lg border bg-slate-100 p-2.5" /></div>{estado.modo === 'CREAR' ? <div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Servicio</label><select required value={servicioId || ''} onChange={(e) => setServicioId(Number(e.target.value))} className="w-full rounded-lg border bg-white p-2.5"><option value="">-- Seleccionar --</option>{servicios.map((item) => <option key={item.id} value={item.id}>{item.codigo} — {item.nombre}</option>)}</select></div> : <div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Servicio</label><input readOnly value={`${tarifa?.servicio_codigo} — ${tarifa?.servicio_nombre}`} className="w-full rounded-lg border bg-slate-100 p-2.5" /></div>}</div>{estado.modo === 'EDITAR' && <div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">Categoría</label><input readOnly value={tarifa?.categoria_nombre || ''} className="w-full rounded-lg border bg-slate-100 p-2.5" /></div>}{estado.modo === 'CREAR' && servicio && <div className="rounded-lg bg-blue-50 p-3 text-sm text-blue-800">Categoría: <strong>{servicio.categoria_nombre}</strong></div>}<div><label className="mb-1 block text-xs font-bold capitalize text-slate-500">{estado.modo === 'EDITAR' ? 'Nuevo precio Faregas' : 'Precio Faregas'}</label><div className="flex"><span className="rounded-l-lg border border-r-0 bg-slate-100 px-3 py-2.5 font-bold">S/</span><input required type="number" min="0.01" step="0.01" value={precio} onChange={(e) => setPrecio(e.target.value)} className="w-full rounded-r-lg border p-2.5" /></div>{tarifa && <p className="mt-1 text-xs text-slate-500">Precio actual: S/ {tarifa.precio.toFixed(2)}</p>}</div><div className="rounded-xl border p-4"><div className="mb-2 flex items-center justify-between"><label className="text-sm font-bold text-slate-700">Producto / SKU de facturación <span className="font-normal text-slate-400">(opcional)</span></label>{producto && <button type="button" onClick={() => { setProducto(null); setError(''); }} className="text-xs font-bold text-red-600">Quitar SKU</button>}</div><input value={skuQuery} onChange={(e) => actualizarBusquedaSku(e.target.value)} placeholder="Buscar por SKU o descripción (mínimo 2 caracteres)" className="w-full rounded-lg border p-2.5 text-sm" />{resultados.length > 0 && <div className="mt-2 max-h-48 overflow-auto rounded-lg border">{resultados.map((item) => {
    const pErr = getProductoError(item, exigeDatosTributarios, categoriaOperacionId);
