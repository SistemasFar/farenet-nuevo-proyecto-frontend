...
export default function TabTarifas() {
  ...
  }), [tarifas, buscar, categoria, estado]);
  ...
  return <div>
    ...
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      ...
      {loading ? <div className="p-10 text-center text-slate-500">Cargando tarifas...</div> : error ? <div className="p-10 text-center text-red-600">{error}</div> : filtradas.length === 0 ? <div className="p-12 text-center"><p className="font-semibold text-slate-600">No existen tarifas configuradas para esta sede.</p><button onClick={() => setModal({ modo: 'CREAR' })} className="mt-4 rounded-lg bg-[#052A79] px-4 py-2 text-sm font-bold text-white">+ ASIGNAR SERVICIO</button></div> : <div className="max-h-[58vh] overflow-auto"><table className="min-w-full text-left text-sm"><thead className="sticky top-0 border-b border-slate-200 bg-white text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Servicio</th><th className="px-4 py-3">Categoría</th><th className="px-4 py-3 text-right">Precio</th><th className="px-4 py-3">SKU Facturación</th><th className="px-4 py-3 text-center">Estado</th><th className="px-4 py-3 text-center">Acciones</th></tr></thead><tbody className="divide-y divide-slate-100">{filtradas.map((tarifa) => <tr key={tarifa.id} className="hover:bg-slate-50"><td className="px-4 py-3"><div className="font-semibold text-slate-800">{tarifa.servicio_nombre}</div><div className="font-mono text-xs text-slate-500">{tarifa.servicio_codigo}</div></td><td className="px-4 py-3">{tarifa.categoria_nombre}</td><td className="whitespace-nowrap px-4 py-3 text-right text-base font-bold text-[#052A79]">S/ {tarifa.precio.toFixed(2)}</td><td className="max-w-sm px-4 py-3">{tarifa.producto_sku ? <><div className="font-mono font-bold">{tarifa.producto_sku}</div><div className="truncate text-xs text-slate-500">{tarifa.producto_descripcion}</div>{tarifa.producto_activo === false && <span className="mt-1 inline-block rounded bg-red-100 px-2 py-0.5 text-xs font-bold text-red-700">SKU INACTIVO</span>}</> : <span className="rounded bg-amber-100 px-2 py-1 text-xs font-bold text-amber-700">SIN SKU ASIGNADO</span>}</td><td className="px-4 py-3 text-center"><span className={`rounded-full px-2 py-1 text-xs font-bold ${tarifa.activo ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{tarifa.activo ? 'ACTIVA' : 'INACTIVA'}</span></td><td className="px-4 py-3 text-center"><div className="flex justify-center gap-3 text-xs font-bold"><button onClick={() => setModal({ modo: 'EDITAR', tarifa })} className="text-[#052A79] hover:underline">Editar</button><button onClick={() => void cambiarEstado(tarifa)} className={tarifa.activo ? 'text-red-600' : 'text-green-600'}>{tarifa.activo ? 'Desactivar' : 'Activar'}</button></div></td></tr>)}</tbody></table></div>}
    </div>
...
function TarifaModal({ estado, sede, onClose, onSaved }: { estado: NonNullable<ModalState>; sede: TarifaSede; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const [producto, setProducto] = useState<ProductoTarifa | null>(tarifa?.producto_facturacion_id ? { id: tarifa.producto_facturacion_id, codigo_sku: tarifa.producto_sku || '', descripcion: tarifa.producto_descripcion || '', unidad: tarifa.producto_unidad, tipo_afectacion_igv: tarifa.producto_afectacion_igv, cuenta_por_cobrar: tarifa.producto_cuenta_por_cobrar, precio_referencia: tarifa.producto_precio_referencia, activo: tarifa.producto_activo !== false } : null);
  ...
  const guardar = async (event: React.FormEvent) => {
    ...
    try { setSaving(true); if (estado.modo === 'CREAR') await faregasTarifasAdminApi.crear({ planta_key: sede.key, servicio_id: servicioId, precio: monto, producto_facturacion_id: producto?.id || null, activo }); else if (tarifa) await faregasTarifasAdminApi.editar(tarifa.id, { precio: monto, producto_facturacion_id: producto?.id || null, activo }); await onSaved(); }
