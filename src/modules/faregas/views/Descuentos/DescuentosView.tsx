...
export function DescuentosView() {
  ...
  const guardar = async () => {
    if (!form.nombre) return void Swal.fire('Complete el formulario', 'El nombre de la campaña o convenio es obligatorio.', 'warning');
    const payload = { ...form };
    try {
      if (editando) {
  ...
  return (
    <div className="space-y-4 pb-8 descuentos-admin">
      ...
      {showForm && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4"><div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white"><div className="flex items-center justify-between border-b px-6 py-4"><div><h2 className="text-lg font-black">{editando ? 'Editar campaña o convenio' : 'Crear campaña o convenio'}</h2><p className="mt-1 text-xs text-slate-500">Paso 1 de 3 · Datos generales y vigencia</p></div><button onClick={() => setShowForm(false)}><X /></button></div><div className="grid gap-4 p-6 md:grid-cols-2">
